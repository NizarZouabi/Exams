...
const AddMovie = (props) => {
    ...
    const submitHandler = (e) => {
        ...
        axios.post("http://localhost:5000/api/movies/new", newMovie, {
            headers: {
    ...
    return (
        <div className='container p-3 border'>
            <div className="d-flex justify-content-between">
                <h3 className='fw-bold ms-2 mt-4'>Submit a Movie and a Review</h3>
                <Logout/>
            </div>
            <form className='container shadow border w-50 p-5 d-flex flex-column gap-3' onSubmit={submitHandler}>
                <label>Movie Title: <input className='form-control' name='title' value={title} onChange={(e) => setTitle(e.target.value)} /></label>
                {title.length <= 1 && title.length !== 0 ? <p className='text-danger'>Title must be more than 1 character</p> : ''}
                ...
                {reviewer.length <= 2 && reviewer.length !== 0 ? <p className='text-danger'>Name must be more than 2 character</p> : ''}
